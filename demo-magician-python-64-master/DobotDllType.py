    ...
    try:
        if masterDevType == DevType.Conntroller:
            if connectInfo.slaveDevInfo1.type == 0 and connectInfo.slaveDevInfo2.type == 0:
                ...
                slaveDevType = 0
                try:
                    fwName = str(connectInfo.masterDevInfo.firmwareName,
                                 encoding="utf-8").strip(b'\x00'.decode())
                    fwVer = str(connectInfo.masterDevInfo.firwareVersion,
                                encoding="utf-8").strip(b'\x00'.decode())
                ...
                fwName = str(connectInfo.slaveDevInfo1.firmwareName, encoding="utf-8").strip(b'\x00'.decode(
                )) if connectInfo.slaveDevInfo1.type != DevType.Idle else str(connectInfo.slaveDevInfo2.firmwareName, encoding="utf-8").strip(b'\x00'.decode())
                fwVer = str(connectInfo.slaveDevInfo1.firwareVersion, encoding="utf-8").strip(b'\x00'.decode()
                                                                                              ) if connectInfo.slaveDevInfo1.type != DevType.Idle else str(connectInfo.slaveDevInfo2.firwareVersion, encoding="utf-8").strip(b'\x00'.decode())
...
def GetDeviceName(api):
    ...
    while (True):
        result = api.GetDeviceName(
            c_int(masterId), c_int(slaveId), szPara,  100)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetDeviceWithL(api, isWithL, version=0, isQueued=0):
    ...
    while (True):
        print(tempSlaveId)
        result = api.SetDeviceWithL(c_int(masterId), c_int(tempSlaveId), c_bool(
            isWithL), c_uint8(version), c_bool(isQueued), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetDeviceWithL(api):
    ...
    while (True):
        result = api.GetDeviceWithL(
            c_int(masterId), c_int(tempSlaveId), byref(isWithL))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetDeviceTime(api):
    time = c_uint32(0)
    while (True):
        result = api.GetDeviceTime(
            c_int(masterId), c_int(slaveId), byref(time))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetDeviceInfo(api):
    ...
    while (True):
        result = api.GetDeviceInfo(
            c_int(masterId), c_int(slaveId), byref(info))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def ResetPose(api, manual, rearArmAngle, frontArmAngle):
    ...
    while (True):
        result = api.ResetPose(c_int(masterId), c_int(
            slaveId), manual, c_rearArmAngle, c_frontArmAngle)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetKinematics(api):
    ...
    while (True):
        result = api.GetKinematics(
            c_int(masterId), c_int(slaveId), byref(kinematics))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetAlarmsState(api,  maxLen=1000):
    ...
    len = c_int(0)
    while (True):
        result = api.GetAlarmsState(c_int(masterId), c_int(
            slaveId), alarmsState, byref(len),  maxLen)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetHOMEParams(api,  x,  y,  z,  r,  isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetHOMEParams(c_int(masterId), c_int(
            slaveId), byref(param),  isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetHOMECmd(api, temp, isQueued=0):
    ...
    if masterDevType == DevType.Magician:
        # 只有Magician
        while (True):
            result = api.SetHOMECmd(c_int(masterId), c_int(
                slaveId), byref(cmd), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
        ...
        while (True):
            result = api.SetHOMECmd(c_int(masterId), c_int(
                slaveDevType), byref(cmd), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetAutoLevelingCmd(api, controlFlag, precision, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetAutoLevelingCmd(c_int(masterId), c_int(
            slaveId), byref(cmd), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetArmOrientation(api,  armOrientation, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetArmOrientation(c_int(masterId), c_int(
            slaveId), armOrientation, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetHHTTrigMode(api, hhtTrigMode):
    while (True):
        result = api.SetHHTTrigMode(
            c_int(masterId), c_int(slaveId), hhtTrigMode)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetHHTTrigMode(api):
    hhtTrigMode = c_int(0)
    while (True):
        result = api.GetHHTTrigMode(
            c_int(masterId), c_int(slaveId), byref(hhtTrigMode))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetHHTTrigOutputEnabled(api, isEnabled):
    while (True):
        result = api.SetHHTTrigOutputEnabled(
            c_int(masterId), c_int(slaveId), isEnabled)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetHHTTrigOutput(api):
    ...
    result = api.GetHHTTrigOutput(
        c_int(masterId), c_int(slaveId), byref(isAvailable))
...
def SetEndEffectorParams(api, xBias, yBias, zBias, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetEndEffectorParams(c_int(masterId), c_int(
            slaveId), byref(param),  isQueued,  byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def GetEndEffectorParams(api):
    param = EndTypeParams()
    while (True):
        result = api.GetEndEffectorParams(
            c_int(masterId), c_int(slaveId), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetEndEffectorLaser(api, enableCtrl,  on, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetEndEffectorLaser(c_int(masterId), c_int(
            slaveId), enableCtrl,  on,  isQueued,  byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetEndEffectorLaser(api):
    ...
    isOn = c_int(0)
    while (True):
        result = api.GetEndEffectorLaser(c_int(masterId), c_int(
            slaveId), byref(isCtrlEnabled),  byref(isOn))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetEndEffectorSuctionCup(api, enableCtrl,  on, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetEndEffectorSuctionCup(c_int(masterId), c_int(
            slaveId), enableCtrl,  on,  isQueued,  byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetEndEffectorGripper(api, enableCtrl,  on, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetEndEffectorGripper(c_int(masterId), c_int(
            slaveId), enableCtrl,  on,  isQueued,  byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def GetEndEffectorGripper(api):
    ...
    isOn = c_int(0)
    while (True):
        result = api.GetEndEffectorGripper(c_int(masterId), c_int(
            slaveId), byref(enableCtrl),  byref(isOn))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetJOGJointParams(api, j1Velocity, j1Acceleration, j2Velocity, j2Acceleration, j3Velocity, j3Acceleration, j4Velocity, j4Acceleration, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetJOGJointParams(c_int(masterId), c_int(
            slaveId), byref(jogParam), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetJOGCoordinateParams(api, xVelocity, xAcceleration, yVelocity, yAcceleration, zVelocity, zAcceleration, rVelocity, rAcceleration, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetJOGCoordinateParams(c_int(masterId), c_int(
            slaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetJOGLParams(api, velocity, acceleration, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetJOGLParams(c_int(masterId), c_int(
            tempSlaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetJOGCommonParams(api, value_velocityratio, value_accelerationratio, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        while (True):
            result = api.SetJOGCommonParams(c_int(masterId), c_int(
                slaveId), byref(param), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
    ...
    if cmd == 0:
        while (True):
            result = api.SetJOGCmd(
                c_int(masterId), c_int(-1), byref(cmdParam), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
        while (True):
            result = api.SetJOGCmd(c_int(masterId), c_int(slaveId), byref(
                cmdParam), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
        ...
        while (True):
            result = api.SetJOGCmd(c_int(masterId), c_int(tempSlaveId), byref(
                cmdParam), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
...
def SetPTPJointParams(api, j1Velocity, j1Acceleration, j2Velocity, j2Acceleration, j3Velocity, j3Acceleration, j4Velocity, j4Acceleration, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetPTPJointParams(c_int(masterId), c_int(
            slaveId), byref(pbParam), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetPTPCoordinateParams(api, xyzVelocity, xyzAcceleration, rVelocity,  rAcceleration,  isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetPTPCoordinateParams(c_int(masterId), c_int(
            slaveId), byref(pbParam), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetPTPLParams(api, velocity, acceleration, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetPTPLParams(c_int(masterId), c_int(
            tempSlaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetPTPJumpParams(api, jumpHeight, zLimit, isQueued=0):
    ...
    while (True):
        result = api.SetPTPJumpParams(c_int(masterId), c_int(
            slaveId), byref(pbParam), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetPTPCommonParams(api, velocityRatio, accelerationRatio, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        while (True):
            result = api.SetPTPCommonParams(c_int(masterId), c_int(
                slaveId), byref(pbParam), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
        ...
        while (True):
            result = api.SetPTPCommonParams(
                c_int(masterId), c_int(-1), byref(pbParam), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetPTPCmd(api, ptpMode, x, y, z, rHead, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetPTPCmd(c_int(masterId), c_int(
            slaveId), byref(cmd), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(2)
            continue
...
def SetPTPWithLCmd(api, ptpMode, x, y, z, rHead, l, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        while (True):
            result = api.SetPTPWithLCmd(c_int(masterId), c_int(
                slaveId), byref(cmd), isQueued, byref(queuedCmdIndex))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(2)
                continue
        ...
        while (True):
            result = api.SetPTPCmd(c_int(masterId), c_int(
                slaveId), byref(cmd1), isQueued, byref(queuedCmdIndex1))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(2)
                continue
            break
...
def SetCPRHoldEnable(api, isEnable):
    while (True):
        result = api.SetCPRHoldEnable(
            c_int(masterId), c_int(slaveId), c_bool(isEnable))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetCPRHoldEnable(api):
    ...
    while (True):
        result = api.GetCPRHoldEnable(
            c_int(masterId), c_int(slaveId), byref(isEnable))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetCPParams(api, planAcc, juncitionVel, acc, realTimeTrack=0,  isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetCPParams(c_int(masterId), c_int(
            slaveId), byref(parm), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetCPCmd(api, cpMode, x, y, z, velocity, isQueued=0):
    ...
    while (True):
        result = api.SetCPCmd(c_int(masterId), c_int(
            slaveId), byref(cmd), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(2)
            continue
...
def SetCP2Cmd(api, cpMode, x, y, z, isQueued=0):
    ...
    while (True):
        result = api.SetCP2Cmd(c_int(masterId), c_int(
            slaveId), byref(cmd), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(2)
            continue
...
def SetCPCommonParams(api, velocityRatio, accelerationRatio, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetCPCommonParams(c_int(masterId), c_int(
            slaveId), byref(pbParam), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetCPLECmd(api, cpMode, x, y, z, power, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetCPLECmd(c_int(masterId), c_int(
            slaveId), byref(cmd), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(2)
            continue
...
def SetARCParams(api,  xyzVelocity, rVelocity, xyzAcceleration, rAcceleration,  isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetARCParams(c_int(masterId), c_int(
            slaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetARCCmd(api, cirPoint, toPoint,  isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetARCCmd(c_int(masterId), c_int(
            slaveId), byref(cmd), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetCircleCmd(api, cirPoint, toPoint,  isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetCircleCmd(c_int(masterId), c_int(
            slaveId), byref(cmd), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetARCCommonParams(api, velocityRatio, accelerationRatio, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetARCCommonParams(c_int(masterId), c_int(
            slaveId), byref(pbParam), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetWAITCmd(api, waitTime, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetWAITCmd(c_int(masterId), c_int(
            slaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetTRIGCmd(api, address, mode,  condition,  threshold,  isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetTRIGCmd(c_int(masterId), c_int(
            slaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetIOMultiplexing(api, address, multiplex, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.SetIOMultiplexing(c_int(masterId), c_int(
            tempSlaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetIODO(api, address, level, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.SetIODO(c_int(masterId), c_int(
            tempSlaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetIOPWM(api, address, frequency, dutyCycle,  isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.SetIOPWM(c_int(masterId), c_int(
            tempSlaveId), byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetEMotor(api, index, isEnabled, speed,  isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.SetEMotor(c_int(masterId), c_int(
            tempSlaveId), byref(emotor), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetEMotorS(api, index, isEnabled, speed, distance,  isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.SetEMotorS(c_int(masterId), c_int(
            tempSlaveId), byref(emotorS), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetAngleSensorStaticError(api,  rearArmAngleError, frontArmAngleError):
    ...
    while (True):
        result = api.SetAngleSensorStaticError(c_int(masterId), c_int(
            slaveId), c_rearArmAngleError, c_frontArmAngleError)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetAngleSensorStaticError(api):
    ...
    while (True):
        result = api.GetAngleSensorStaticError(c_int(masterId), c_int(
            slaveId), byref(rearArmAngleError),  byref(frontArmAngleError))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetAngleSensorCoef(api,  rearArmAngleCoef, frontArmAngleCoef):
    ...
    while (True):
        result = api.SetAngleSensorCoef(c_int(masterId), c_int(
            slaveId), c_rearArmAngleCoef, c_frontArmAngleCoef)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetAngleSensorCoef(api):
    ...
    while (True):
        result = api.GetAngleSensorCoef(c_int(masterId), c_int(
            slaveId), byref(rearArmAngleCoef),  byref(frontArmAngleCoef))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetWIFIPassword(api):
    ...
    while (True):
        if not QuitDobotApiFlag:
            break
        result = api.GetWIFIPassword(
            c_int(masterId), c_int(slaveId), szPara,  25)
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetWIFINetmask(api, addr1,  addr2,  addr3,  addr4):
    ...
    while (True):
        if not QuitDobotApiFlag:
            break
        result = api.SetWIFINetmask(
            c_int(masterId), c_int(slaveId), byref(wifiNetmask))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetWIFIGateway(api, addr1,  addr2,  addr3,  addr4):
    ...
    while (True):
        if not QuitDobotApiFlag:
            break
        result = api.SetWIFIGateway(
            c_int(masterId), c_int(slaveId), byref(wifiGateway))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetWIFIDNS(api, addr1,  addr2,  addr3,  addr4):
    ...
    while (True):
        if not QuitDobotApiFlag:
            break
        result = api.SetWIFIDNS(
            c_int(masterId), c_int(slaveId), byref(wifiDNS))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetColorSensor(api, isEnable, colorPort, version=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.SetColorSensor(c_int(masterId), c_int(
            tempSlaveId), enable, port, version, 1, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetColorSensor(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.GetColorSensor(c_int(masterId), c_int(
            tempSlaveId), byref(r),  byref(g),  byref(b))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetInfraredSensor(api,  isEnable, infraredPort, version=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.SetInfraredSensor(c_int(masterId), c_int(
            tempSlaveId), enable, port, version, 1, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetLostStepParams(api, threshold, isQueued=0):
    ...
    t = c_float(threshold)
    while (True):
        result = api.SetLostStepParams(c_int(masterId), c_int(
            slaveId), t, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetLostStepCmd(api, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetLostStepCmd(c_int(masterId), c_int(
            slaveId), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetPTPJointParamsEx(api, j1Velocity, j1Acceleration, j2Velocity, j2Acceleration, j3Velocity, j3Acceleration, j4Velocity, j4Acceleration, isQueued=0):
    ret = SetPTPJointParams(api, j1Velocity, j1Acceleration, j2Velocity, j2Acceleration,
...
def GetUpgradeFWReadyCmd(api, fwSize, md5):
    ...
    if masterDevType == DevType.Conntroller and (slaveDevType == DevType.MagicianLite or slaveDevType == DevType.Idle):
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.GetUpgradeFWReadyCmd(c_int(masterId), c_int(
            tempSlaveId), byref(upgradeFWReadyCmd), byref(isUpgrade))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetIOMultiplexingExt(api, addr):
    ...
    while (True):
        result = api.GetIOMultiplexing(
            c_int(masterId), c_int(-1), byref(param))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetIOPWMExt(api, address, frequency, dutyCycle,  isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetIOPWM(c_int(masterId), c_int(-1),
                              byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetIODOExt(api, address, level, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetIODO(c_int(masterId), c_int(-1),
                             byref(param), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetEMotorExt(api, index, isEnabled, speed, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetEMotor(c_int(masterId), c_int(-1),
                               byref(emotor), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetColorSensorExt(api, isEnable, colorPort, version=0, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetColorSensor(c_int(
            masterId), c_int(-1), enable, port, version, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetInfraredSensorExt(api,  isEnable, infraredPort, version=0, isQueued=0):
    ...
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetInfraredSensor(c_int(
            masterId), c_int(-1), enable, port, version, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def GetInfraredSensorExt(api, infraredPort):
    ...
    while (True):
        result = api.GetInfraredSensor(
            c_int(masterId), c_int(-1), port,  byref(value))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetSeeedColorSensorExt(api):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.GetSeeedColorSensor(c_int(masterId), c_int(
            tempSlaveId), byref(r),  byref(g),  byref(b), byref(Cct))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetSeeedColorSensorExt(api, SeeedPort, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.SetSeeedColorSensor(c_int(masterId), c_int(
            tempSlaveId), port, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetSeeedTempSensorExt(api, SeeedPort, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.SetSeeedTempSensor(c_int(masterId), c_int(
            tempSlaveId), port, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetSeeedLightSensorExt(api, SeeedPort, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.SetSeeedLightSensor(c_int(masterId), c_int(
            tempSlaveId), port, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetSeeedRgbExt(api, SeeedPort, Rgb, isQueued=0):
    ...
    if slaveDevType == DevType.Magician:
        ...
        tempSlaveId = slaveId
    while (True):
        result = api.SetSeeedRgb(c_int(masterId), c_int(
            tempSlaveId), port, rgb, isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetLostStepEnableAndParamsCmd(api, enable, threshlod, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetLostStepEnableAndParamsCmd(c_int(masterId), c_int(
            slaveId), c_uint8(enable), c_float(threshlod), isQueued, byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetEndEffectorType(api, endType=0, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetEndEffectorType(c_int(masterId), c_int(
            slaveId), isQueued, c_uint8(endType), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetEndEffectorType(api):
    endType = c_uint8(0)
    while (True):
        result = api.GetEndEffectorType(
            c_int(masterId), c_int(slaveId), byref(endType))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetServoAngle(api, servoId, angle, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetServoAngle(c_int(
            masterId), c_int(-1), isQueued, c_uint8(servoId), c_float(angle), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetArmSpeedRatio(api, paramsMode, speedRatio, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetArmSpeedRatio(c_int(masterId), c_int(slaveId), isQueued, c_uint8(
            paramsMode), c_uint8(speedRatio),  byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def GetArmSpeedRatio(api, paramsMode=0):
    ...
    while (True):
        result = api.GetArmSpeedRatio(c_int(masterId), c_int(
            slaveId),  c_uint8(paramsMode), byref(speedRatio))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
        break
...
def SetLSpeedRatio(api, paramsMode, speedRatio, isQueued=0):
    queuedCmdIndex = c_uint64(0)
    while (True):
        result = api.SetLSpeedRatio(c_int(masterId), c_int(-1), isQueued, c_uint8(
            paramsMode), c_uint8(speedRatio), byref(queuedCmdIndex))
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
